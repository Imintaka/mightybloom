...
- Рабочие дни: пользователь отмечает в календаре вручную.
...
type WorkDaysByMonth = Record<string, string[]>; // YYYY-MM -> dates
...
2. `Month`: круговой календарь сна (дни месяца), индикаторы сна/стикера/рабочего дня и открытие дня для редактирования.
